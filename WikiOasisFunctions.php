...
class WikiOasisFunctions {
    ...
    public static function getCurrentSuffix(): string {
        ...
        // Return the first suffix as fallback
        return array_key_first( self::SUFFIXES );
    }
    ...
    public function isBeta(): bool {
        return str_ends_with( $this->server, '.betaoasis.xyz' );
    }
    ...
    public static function writeToCache( string $cacheShard, array $configObject ): void {
        ...
        if ( $tmpFile !== false ) {
            $contents = StaticArrayWriter::write( $configObject, 'Automatically generated by WikiOasisFunctions' );
    ...
    public static function getActiveExtensions( bool $useConfigCache = true ): array {
        ...
        $allExtensions = [];
        foreach ( $wgManageWikiExtensions as $key => $data ) {
            $name = $data['name'] ?? null;
            if ( is_string( $name ) && $name !== '' ) {
                $allExtensions[$name] = $key;
            }
        }
